    ...
    */
    ...
    public function registerAction()
    {
        if($this->isPost())
        {
            ...
            elseif(model('manager')->field('uid')->where("username='{$username}'")->find())
            {
                $message = '学(工)号已存在.';
            }
    ...
    public function checkAction()
    {
        ...
        if($where)
        {
            $data = model('manager')->field('uid')->where($where)->find();
