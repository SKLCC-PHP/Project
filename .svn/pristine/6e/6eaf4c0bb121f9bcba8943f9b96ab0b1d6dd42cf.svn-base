...
class Log
{
    ...
    private static function accessInfo()
    {
        $separator = self::getSeparator();
        $now = date(self::$format);
        $uid = (int)$_SESSION[SESSION_USER_NAME]['uid'];
        $ip = get_client_ip();
        RunTime::stop();
        $spent = RunTime::spent();
        
        $message = "[$now]".$separator.$uid.$separator.$ip.$separator.CONTROLLER_NAME.$separator.ACTION_NAME.$separator.HttpRequest::getServerName().':'.HttpRequest::getServerPort().$separator.$spent."\r\n";
        return $message;
    }
