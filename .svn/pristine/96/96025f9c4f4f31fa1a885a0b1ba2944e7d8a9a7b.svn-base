...
class ManagerModel extends Model
{
    ...
    public function sendVerifyEmail()
    {
        ...
        $url = $host.url(__PUB__.'member/index.php?c=user&a=active&k='.$activationkey.'&u='.$username);
    ...
    public function sendfindPwdEmail($username, $password, $email)
    {
        ...
        $cfg_webname = getCfgVar('cfg_webname');
        $cfg_adminemail = getCfgVar('cfg_adminemail');
        $url = $host.url(__PUB__.'member/index.php?c=user&a=resetPwd&key='.base64_encode($username.'.'.Date::getTimeStamp().'.'.md5($username.'+'.$password)));
        $message = '<p style="font-size:14px;">亲爱的用户'.$username.'：</p>
